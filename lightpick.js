...
function isWeekend(date) {
    return date.isoWeekday() === weekNameDays.SATURDAY || date.isoWeekday() === weekNameDays.SUNDAY;
}
...
(function (root, factory) {
    ...
    var document = window.document,
        ...
        renderDay = function (opts, date, dummy, extraClass) {
            ...
            if (opts.disableWeekends && isWeekend(date)) {
                day.className.push('is-disabled');
            }

            if (opts.colorWeekend && isWeekend(date)) {
                day.className.push('is-weekend');
            }

            var eventDescription = null;
            if (opts.eventDays != null) {
                var eventDays = opts.eventDays;
                for (var ed = 0; ed < eventDays.length; ed++) {
                    if (eventDays[ed].everyYear && moment(eventDays[ed].date).format("MM-DD") == date.format("MM-DD") || moment(eventDays[ed].date).format("YYYY-MM-DD") == date.format("YYYY-MM-DD")) {
                        eventDescription = eventDays[ed].eventDescription;
            ...
            if (eventDescription)
                div.setAttribute('title', eventDescription);
